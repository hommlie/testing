                ...
                <div class="col-12">
                    ...
                    <div class="card">                       
                        ...
                        <div class="card-body p-3">
                            <form action="{{ route('admin.orders.updateorder',$data->id) }}" method="post" enctype="multipart/form-data">
                                @csrf
                                @method('PUT')
                                ...
                                <input type="hidden" name="hidden_product_name" id="hidden_product_name" value="{{ $data->product_name }}">
                                ...
                                <div class="row mb-3">
                                    <div class="col-lg-6">
                                        ...
                                        <select name="subcategory" id="order_subcategory" class="form-control">
                                            ...
                                            @foreach ($subcategory as $subcat)
                                                <option value="{{ $subcat->id }}" {{ $subcat->id ==   $getCategoryId->subcat_id ? 'selected' : '' }}>{{ $subcat->subcategory_name }}</option>
                                            @endforeach 
                                        </select>
                                        ...
                                        <div class="row">
                                                ...
                                                <select name="order_service_type" id="order_service_type" class="form-control">
                                                    ...
                                                    @foreach ($variations as $variation)
                                                        @if ($variation->attribute_id == 2)
                                                            <option value="{{ $variation->variation }}" {{ isset($ExplodedVariations[0]) && $variation->variation == $ExplodedVariations[0] ? 'selected' : '' }}>{{ $variation->variation }}</option>
                                            ...
                                            <div class="col-lg-6">
                                                <select name="order_service_area" id="order_service_area" class="form-control">
                                                    ...
                                                    @foreach ($variations as $variation)
                                                        @if ($variation->attribute_id == 5)
                                                            <option value="{{ $variation->variation }}" {{ isset($ExplodedVariations[1]) && $variation->variation == $ExplodedVariations[1] ? 'selected' : '' }}>{{ $variation->variation }}</option>
                                        ...
                                        <span class="border rounded p-2">
                                            ...
                                            <span class="p-2 quantity-value">{{ $data->qty }}</span>
                                            <span class="px-2 bg-light increment" style="cursor: pointer;">+</span>
                                        </span>
                                        <br><br>
                                        <label for="price">Price (Without GST)</label>
                                        <input type="text" name="price" placeholder="Price"  id="order_price" value="{{ $data->price }}" readonly class="form-control" />
                                        @if ($errors->has('price'))
                                        ...
                                        <div class="row">
                                            <div class="col-lg-6">
                                                ...
                                                <input type="date" name="desired_date" placeholder="desired_date" value="{{ $data->desired_date }}"  id="desired_date"  class="form-control" />
                                            ...
                                            <div class="col-lg-6">
                                                ...
                                                <input type="time" name="desired_time" placeholder="desired_time" value="{{ $data->desired_time }}"  id="desired_time"  class="form-control" />
                                    ...
                                    <div class="col-lg-6">
                                        ...
                                        <input type="text" name="fullname" placeholder="Enter Customer name" value="{{ $data->full_name }}"  id="fullname" class="form-control" />
                                        @if ($errors->has('fullname'))
                                            <span class="text-danger">{{ $errors->first('fullname') }}</span>
                                        @endif
                                        <br>
                                        <label for="email">Email</label>
                                        <input type="email" name="email" placeholder="Enter Customer email"  id="email" value="{{ $data->email }}" class="form-control" />
                                        @if ($errors->has('email'))
                                            <span class="text-danger">{{ $errors->first('email') }}</span>
                                        @endif
                                        <br>
                                        <label for="mobile">Mobile Number</label>
                                        <input type="text" name="mobile" placeholder="Enter Customer mobile Number" value="{{ $data->mobile }}"  id="mobile" class="form-control" />
                                        @if ($errors->has('mobile'))
                                            <span class="text-danger">{{ $errors->first('mobile') }}</span>
                                        @endif
                                        <br>
                                        <label for="landmark">Landmark</label>
                                        <input type="text" name="landmark" placeholder="Enter Customer landmark"  id="landmark" value="{{ $data->landmark }}" class="form-control" />
                                        @if ($errors->has('landmark'))
                                            <span class="text-danger">{{ $errors->first('landmark') }}</span>
                                        @endif
                                        <br>
                                        <label for="address">Address</label>
                                        <input type="text" name="address" placeholder="Enter Customer address"  id="address" value="{{ $data->street_address }}" class="form-control" />
                                        @if ($errors->has('address'))
                                            <span class="text-danger">{{ $errors->first('address') }}</span>
                                        @endif
                                        <br>
                                        <label for="pincode">Pincode</label>
                                        <input type="number" name="pincode" placeholder="Enter Customer pincode" value="{{ $data->pincode }}"  id="pincode" class="form-control" />
                                        @if ($errors->has('pincode'))
                                            <span class="text-danger">{{ $errors->first('pincode') }}</span>
                                        @endif
                                        <br>
                                        <label for="pincode">Total Price</label>
                                        <input type="text" name="order_total" placeholder="Total Amount" value="{{ $data->order_total }}"  id="order_total" class="form-control" />
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const totalPriceDisplay = document.querySelector('#order_total');
