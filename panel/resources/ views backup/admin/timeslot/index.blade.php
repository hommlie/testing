                ...
                <div class="col-12">
                    ...
                    <div class="card">                       
                        <div class="card-header">
                            <h4 class="card-title">{{ trans('Time Slots') }}</h4>
                            <a href="{{route('admin.timeslot.add')}}" class="btn btn-raised btn-primary btn-min-width mr-1 mb-1 float-right" style="margin-top: -30px;">
                                {{ trans('Add Time Slots') }}
                            </a>
                        </div>
                        ...
                        <div class="card-body collapse show">
                            <div class="card-block card-dashboard" id="table-display">
                                <div id="filter_buttons" class="d-flex align-items-center">
                                    ...
                                    <a href="timeslot" class="btn btn-sm btn-outline-success round-pill mx-2" id="filter_reset">Reset Filter</a>
                                </div>
                                ...
                                <br>
                                    @include('admin.timeslot.timeslottable')
                            </div>
                        </div>
                    </div>
                </div>
...
<script type="text/javascript">
    ...
    $(document).ready(function() {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: '{{ route("admin.timeslot.delete") }}',
                    type: 'POST',
                    data: {
                        id: id,
                        _token: '{{ csrf_token() }}'
    ...
    $('.deactive-btn').on('click', function(e) {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: '{{ route("admin.timeslot.deactive") }}',
                    type: 'POST',
                    data: {
                        id: id,
                        _token: '{{ csrf_token() }}'
