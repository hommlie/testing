...
export default function ProductPage() {
    ...
    async function getProductReview(id) {
        try {
            const response = await axios.post(`${config.API_URL}/api/productreview`, { product_id: id });
    ...
    useEffect(() => {
        if (prodData && prodData.variations) {
            ...
            if (attrs.length > 0) {
                ...
                const firstVariation = prodData.variations.find(v => v.attribute_name === attrs[0]);
                if (firstVariation) {
    ...
    useEffect(() => {
        const calculatePrice = () => {
            if (selectedVariation) {
                ...
                const tax = prodData.tax_type === "amount" 
                    ? Number(prodData.tax) 
    ...
    return (
        <main className="main-div flex flex-col lg:flex-row p-4 lg:space-x-4 mb-2 scroll-smooth">
            {isLoading ? (
                ...
                <section className="bg-white p-4 mb-6">
                    <div className="relative">
                        ...
                        <>
                            ...
                            <button onClick={handleNextMedia} className="absolute top-1/2 right-2 transform -translate-y-1/2 bg-black bg-opacity-50 text-white p-2 rounded-full">
                                <IoIosArrowForward />
                            </button>
                        </>
                        )}
                    </div>
                    <div className="flex flex-col md:flex-row gap-4 justify-between pb-4 mb-4 mt-10" style={{borderBottom: "1px solid #E3E3E3"}}>
                        <div className="space-y-1 sm:space-y-3 lg:space-y-4">
                            <p className="text-4xl sm:text-5xl font-bold mb-2">{prodData?.product_name}</p>
                            <div className="flex items-center mb-2">
                                <MdStars className="text-2xl" color="#FF3269" />
                                <p className="text-lg sm:text-xl font-normal ml-2">{reviewData?.avg_ratting} ({reviewData?.total} reviews)</p>
                            </div>
                            <p className="flex items-center">
                            <span className="text-3xl font-bold">
                                    ₹{selectedVariation ? selectedVariation.discounted_variation_price : prodData?.discounted_price}
                                </span>
                                <span className="line-through text-2xl sm:text-3xl font-light ml-4" style={{color: "#545454"}}>
                                    ₹{selectedVariation ? selectedVariation.price : prodData?.product_price}
                                </span>
                                {prodData?.est_shipping_days != 0 ?
                                    <span className=" flex flex-row items-center gap-2 text-xl sm:text-2xl font-normal ml-8" style={{color: "#545454"}}>
                                        <CiClock1 />
                ...
                <section className="bg-white rounded-lg p-4">
                    <ul className="flex mb-3" style={{borderBottom: "1px solid rgba(0, 0, 0, 0.1)"}}>
                        <li className={`mr-1 ${activeTab === 1 ? 'border-[#035240]' : ''}`}>
                            <button
                                className={`inline-block py-2 px-4 text-[#035240] ${activeTab === 1 ? 'font-medium' : ''}`}
                                onClick={() => setActiveTab(1)}
                                style={{ borderBottom: activeTab === 1 ? "2px solid #20B526" : "none" }}
                                >
                                Descriptions
                            </button>
                        </li>
                        <li className={`mr-1 ${activeTab === 2 ? 'border-[#035240]' : ''}`}>
                            <button
                                className={`hidden sm:inline-block py-2 px-4 text-[#035240] ${activeTab === 2 ? 'font-medium' : ''}`}
                                onClick={() => setActiveTab(2)}
                                style={{ borderBottom: activeTab === 2 ? "2px solid #20B526" : "none" }}
                            >
                                Frequently Ask Question
                            </button>
                            <button
                                className={`inline-block sm:hidden py-2 px-4 text-[#035240] ${activeTab === 2 ? 'font-medium' : ''}`}
                                onClick={() => setActiveTab(2)}
                                style={{ borderBottom: activeTab === 2 ? "2px solid #20B526" : "none" }}
                            >
                                FAQs
                        ...
                        <li className={`mr-1 ${activeTab === 3 ? 'border-[#035240]' : ''}`}>
                            <button
                                className={`hidden sm:inline-block py-2 px-4 text-[#035240] ${activeTab === 3 ? 'font-medium' : ''}`}
                                onClick={() => setActiveTab(3)}
                                style={{ borderBottom: activeTab === 3 ? "2px solid #20B526" : "none" }}
                            >
                                Customer Feedback
                            </button>
                            <button
                                className={`inline-block sm:hidden py-2 px-4 text-[#035240] ${activeTab === 3 ? 'font-medium' : ''}`}
                                onClick={() => setActiveTab(3)}
                                style={{ borderBottom: activeTab === 3 ? "2px solid #20B526" : "none" }}
                            >
                                Reviews
                                        ...
                                        <div className="flex items-start space-x-4">
                                            ...
                                            <div className="flex-grow">
                                                <div className="flex justify-between items-center mb-2">
                                                    <span className="font-semibold text-gray-800">{user?.name ? user?.name : "Unknown"}</span>
                                                    <span className="text-sm text-gray-500">{new Date(ratting?.created_at).toLocaleDateString()}</span>
                                                </div>
                                                <div className="flex items-center mb-2">
                                                        ...
                                                        if (index != ratting?.ratting) {
                                                            return (
                                                                <svg
                                                                    key={star}
                                                                    className={`w-5 h-5 ${
                                                                        star <= ratting?.ratting ? 'text-yellow-400' : 'text-gray-300'
                                                                    }`}
                                                                    fill="currentColor"
                                                                    viewBox="0 0 20 20"
                                                    ...
                                                    <span className="ml-2 text-sm font-medium text-gray-500">{ratting?.ratting} out of 5</span>
                                                </div>
                                                <p className="text-gray-700 mt-2">"{ratting?.comment}"</p>
                                                {ratting?.assigned_to ?
                                                    <p className="text-gray-700 mt-2">Technician: {ratting?.assigned_to}</p>
                                                    : null
                                                }
                                            </div>
                                        </div>
                    ...
                    <div className="flex justify-between items-center">
                        ...
                        <div className="flex flex-row items-center">
                            <p 
                                ...
                                style={{backgroundColor: `${cartLength == undefined || cartLength === 0 ? '#929B9B' : '#FF3269'}`}}
                            >
                                {cartLength == undefined || cartLength === 0 ? 0 : cartLength}
                            </p>
                            ...
                            <div className="mt-4 p-4 bg-gray-100 rounded-lg">
                                ...
                                {selectedVariation.variation_times && selectedVariation.variation_interval && (
                                    ...
                                    <p className="text-sm text-gray-600">
                                        No. of Services : {selectedVariation.variation_times} Times
                                    </p>
                                    <p className="text-sm text-gray-600">
                                        Scheduled every : {selectedVariation.variation_interval} Days
                                    </p>
                                    </>
                                )}
                            </div>
