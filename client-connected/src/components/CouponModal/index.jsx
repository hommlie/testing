...
const CouponModal = ({ isOpen, onClose, totalAmount, cat_id }) => {
  ...
  async function getCoupons() {
    ...
    await axios
      .post(`${config.API_URL}/api/coupons`, {
        cat_id,
      })
      .then((response) => {
  ...
  const handleApply = () => {
    const coupon = filteredCoupons.find(
      (cp) => cp.coupon_name.toLowerCase() === searchTerm.toLowerCase()
    );
    if (coupon) {
  ...
  const applySelectedCoupon = (coupon) => {
    ...
    localStorage.setItem(
      ...
      JSON.stringify({ ...coupon, calculatedDiscount: discount })
    );
