        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-3 lg:grid-cols-3 gap-4">
          {items?.map((item, index) => (
            <div
              ...
              key={index}
              className="max-w-xs h-full mx-auto flex flex-col gap-2 p-4 justify-between hover:shadow-xl rounded-lg"
            >
              <div className="">
                <img
                  className="w-60 h-[100px] sm:h-56 object-cover"
                  src={item?.productimage?.image_url}
                  alt="product"
                />
              </div>
              <h5 className="text-base font-semibold text-gray-900 w-full">
                {item?.product_name}
              </h5>
