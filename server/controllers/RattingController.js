...
exports.getGoogleReviews = async (req, res) => {
  try {
    const response = await axios.get(
      "https://maps.googleapis.com/maps/api/place/details/json?place_id=ChIJgxuWZf0_rjsRbJaur3y9Lxo&fields=reviews&key=AIzaSyBmaR3DSseRPUCCvGT0Ru8aK-Jrm39NlTE"
    );
    return res
      .status(200)
...
exports.addRatting = async (req, res) => {
  ...
  if (!user_id) {
    return res
      .status(400)
      .json({ status: 0, message: "Please login to add Wishlist" });
  }

  if (!ratting) {
    return res
      .status(400)
      .json({ status: 0, message: "Please select the rattings" });
  }
  ...
  try {
    ...
    if (!order?.assigned_to) {
      return res
        .status(400)
        .json({ status: 0, message: "Order is not assigned to any employee" });
    }
...
exports.productReview = async (req, res) => {
  ...
  try {
    ...
    const fiveRatting = await Ratting.findAll({
      where: { product_id, ratting: 5 },
    });
    const fourRatting = await Ratting.findAll({
      where: { product_id, ratting: 4 },
    });
    const threeRatting = await Ratting.findAll({
      where: { product_id, ratting: 3 },
    });
    const twoRatting = await Ratting.findAll({
      where: { product_id, ratting: 2 },
    });
    const oneRatting = await Ratting.findAll({
      where: { product_id, ratting: 1 },
    });
    ...
    const allReview = await Ratting.findAll({
      ...
      include: [
        ...
        },
        {
          ...
          attributes: [
            ...
            [
              sequelize.literal(
                `CONCAT('${apiUrl}/storage/app/public/images/reviews/', image)`
              ),
              "image_url",
            ],
          ],
      ...
      attributes: [
        ...
        [
          sequelize.fn(
            "DATE_FORMAT",
            sequelize.col("Ratting.created_at"),
            "%d-%m-%Y"
          ),
          "date",
        ],
    ...
    const rattings = {
      avg_ratting:
        avgRatting.reduce((acc, r) => acc + r.ratting, 0) / avgRatting.length,
