...
exports.coupons = async (req, res) => {
    ...
    try {
        ...
        return res.status(500).json({ status: 0, message: "Failed to fetch coupons", error });
    }
};
...
exports.applycoupons = async (req, res) => {
    ...
    try {
        ...
        return res.status(500).json({ status: 0, message: "Failed to apply coupon", error });
    }
};
...
exports.getCoupons = async (req, res) => {
    try {
        ...
        if (couponsdata.length > 0) {
            return res.status(200).json({ status: 1, message: 'Success', data: couponsdata });
        } else {
            return res.status(200).json({ status: 0, message: 'No data found' });
        }
    } catch (error) {
