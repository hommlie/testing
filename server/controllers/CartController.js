...
exports.addToCart = async(req, res) => {
    const { user_id, product_id, vendor_id, product_name, image, qty, price, attribute, variation, tax, shipping_cost } = req.body;
    ...
    try {
      ...
      if (!existingCart) {
        ...
        if (existingCart.vendor_id !== vendor_id && existingCart.user_id === user_id) {
          return res.status(400).json({ status: 0, message: 'First empty your Cart, Then add new product in Cart' });
        } else {
...
exports.getCart = async(req, res) => {
    ...
    try {
      ...
      const cartItems = await Cart.findAll({
        attributes: ['id', 'user_id', 'product_id', 'product_name', 'vendor_id', 'qty', 'price', 'attribute', 'attribute_name', 'variation', 'variation_name', 'tax', 'shipping_cost', 
      ...
      if (cartItems.length > 0) {
        return res.status(200).json({ status: 1, message: 'Success', data: cartItems });
      } else {
        return res.status(200).json({ status: 0, message: 'No items found in the cart' });
      }
    } catch (error) {
      return res.status(500).json({ status: 0, message: 'Something went wrong', error: error.message });
    }
...
exports.deleteProduct = async(req, res) => {
    ...
    try {
      ...
      if (!cart_id) {
        return res.status(400).json({ status: 0, message: 'Cart id is required' });
      }
