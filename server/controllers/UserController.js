...
exports.registerOrLogin = async (req, res) => {
  ...
  try {    
      ...
      var options = {
        ...
        params: {
          otp_expiry: '3',
          template_id: '66b09df3d6fc0561e241f922',
          mobile: mobile,
      ...
      axios.request(options).then(function (response) {
        ...
        if (response.data.type === 'success') {
          ...
          return res.status(200).json({ status: 0, message: response.data.message });
        }
      }).catch(function (error) {
        console.error(error);
        return res.status(200).json({ status: 0, message: 'OTP generation failed', error });
      })
...
exports.verifyOtp = async (req, res) => {
  ...
  try {
    ...
    axios.request(options).then(function (response) {
      ...
      return res.status(200).json({ status: 0, message: 'OTP verification failed', error });
    });
...
exports.reSendOtp = async (req, res) => {
  ...
  if (!mobile) {
      return res.status(200).json({ status: 0, message: 'Mobile number is required' });
  }
  ...
  try {
    ...
    var options = {
      ...
      params: {authkey: '403754ASWGpJz366b09ec2P1', retrytype: 'text', mobile: mobile}
    };
    ...
    axios.request(options).then(function (response) {
      ...
      console.error(error);
      return res.status(200).json({ status: 0, message: 'OTP resend failed', error });
    });
...
exports.editProfile = async(req, res) => {
    ...
    if (!user_id || !name ) {
      return res.status(400).json({ status: 0, message: 'User ID and name are required' });
    }
    ...
    return res.status(200).json({ status: 1, message: 'Profile updated', data: userJson });
}

exports.help = async(req, res) => {
    const { user_id, first_name, last_name, mobile, email, subject, message } = req.body;

    if (!user_id || !first_name || !last_name || !mobile || !email || !subject || !message) {
